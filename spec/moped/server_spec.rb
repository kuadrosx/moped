...
describe Moped::Server do
  ...
  describe "#resolve_address" do
    ...
    context "when given an ip:port address" do
      it "returns an ip and port tuple" do
        server.resolve_address("127.0.0.1:123").should \
          eq ["127.0.0.1", 123]
      end
    end

    context "when given a host:port address" do
      it "returns an ip and port tuple" do
        server.resolve_address("localhost:123").should \
          eq ["127.0.0.1", 123]
      end
    end
  end
